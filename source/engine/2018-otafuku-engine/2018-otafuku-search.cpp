	...
	Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning)
	{
		...
		if (!rootNode)
		{
			...
			if (Threads.stop.load(std::memory_order_relaxed) || (ss->ply >= MAX_PLY || pos.game_ply() > Limits.max_game_ply)
				// またはスレッド毎のノード数制限を超えている
				|| (Search::Limits.nodes_per_thread /*node制限あり*/ && thisThread->nodes.load(std::memory_order_relaxed) >= Search::Limits.nodes_per_thread))
...
namespace Learner
{
	...
	ValueAndPV search(Position& pos, int depth_, size_t multiPV /* = 1 */, u64 nodesLimit /* = 0 */)
	{
		...
		Search::Limits.nodes_per_thread = nodesLimit;
			...
			)
		{
			...
			for (pvIdx = 0; pvIdx < multiPV && !Threads.stop
				// node制限を超えた場合もこのループを抜ける
				// 探索ノード数は、この関数の引数で渡されている。
				&& !(nodesLimit /*node制限あり*/ && th->nodes.load(std::memory_order_relaxed) >= nodesLimit); ++pvIdx)
			{
				// それぞれのdepthとPV lineに対するUSI infoで出力するselDepth
				selDepth = 0;

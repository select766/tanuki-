...
namespace {
  ...
  constexpr char* kBookShowProgressPerPositions = "BookShowProgressPerPositions";
...
bool Book::Initialize(USI::OptionsMap& o) {
  o[kBookSfenFile] << Option("wdoor.sfen");
  o[kBookMaxMoves] << Option(32, 0, 256);
  o[kBookSearchDepth] << Option(24, 0, 256);
  o[kBookShowProgressPerPositions] << Option(1000, 1, std::numeric_limits<int>::max());
...
bool Book::CreateScoredBook() {
  ...
  int show_progress_per_positions = (int)Options[kBookShowProgressPerPositions];
...
#pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < sfens.size(); ++i) {
    ...
    Utility::ShowProgress(start_time, position_index, num_positions, show_progress_per_positions);

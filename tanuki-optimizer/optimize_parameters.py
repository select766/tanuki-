...
# Preparation
#
# 1. Install Python 3.8.0 64-bit ver
# - Python 3.8.0 64bit
# -- http://www.python.org
#
...
class YaneuraouBuilder(object):
    ...
    def build(self, args):
        ...
        with open(COMMANDLINE_ARGS.temporary_parameter_file_path, "w") as f:
            f.write("#ifndef YANEURAOU_PARAM_H_INCLUDED\n")
            f.write("#define YANEURAOU_PARAM_H_INCLUDED\n")
...
def function(args):
    ...
    if START_COUNTER < CURRENT_COUNTER:
        ...
        remaining = datetime.timedelta(
            seconds=sec_per_one * (COMMANDLINE_ARGS.max_evals - CURRENT_COUNTER)
        )
        print(
            CURRENT_COUNTER, "/", COMMANDLINE_ARGS.max_evals, str(remaining), flush=True
        )
    CURRENT_COUNTER += 1
    ...
    engine_invoker_args = [
        ...
        "--engine1-file-path",
        COMMANDLINE_ARGS.engine1,
        "--engine2-file-path",
        COMMANDLINE_ARGS.engine2,
        "--eval1-folder-path",
        COMMANDLINE_ARGS.eval_dir1,
        "--eval2-folder-path",
        COMMANDLINE_ARGS.eval_dir2,
        "--num-concurrent-games",
        str(COMMANDLINE_ARGS.num_threads),
        "--num-games",
        str(COMMANDLINE_ARGS.num_games),
        "--hash-mb",
        str(COMMANDLINE_ARGS.hash),
        "--nodes1",
        ...
        "24",
        "--sfen-file-path",
        COMMANDLINE_ARGS.sfen_file_path,
        "--gui",
        "False",
        ...
        "--book-eval-diff2",
        "10000",
        "--consider-book-move-count1",
        "true",
        "--consider-book-move-count2",
        "true",
    ]
    ...
    if (
        COMMANDLINE_ARGS.store_interval > 0
        and STATE.get_n_accumulated_iterations() % COMMANDLINE_ARGS.store_interval == 0
    ):
...
def main():
    ...
    parser.add_argument(
        "--max_evals", type=int, default=1000, help="max evaluation for hyperopt."
    )
    parser.add_argument(
        "--num_threads", type=int, default=48, help="number of threads."
    )
    parser.add_argument(
        "--num_games", type=int, default=48, help="number of games."
    )
    parser.add_argument(
        "--num_searched_nodes",
        type=int,
    ...
    parser.add_argument(
        "--hash", type=int, default=1024, help="Transposition table hash size."
    )
    parser.add_argument(
    ...
    space = [
        hp.quniform(parameter.name, parameter.min, parameter.max, 1)
        for parameter in PARAMETERS
    ]

    STATE = HyperoptState()
    STATE_STORE_PATH = "optimize_parameters.hyperopt_state.{}.pickle".format(
    ...
    best = fmin(
        ...
        space,
        algo=tpe.suggest,
        max_evals=STATE.calc_max_evals(COMMANDLINE_ARGS.max_evals),
        trials=STATE.get_trials(),
    )
